...
DescriptorMatcher::DescriptorMatcher()
    : d_matcherType(initData(&d_matcherType, "matcher",
      ...
      d_matchingAlgo(initData(&d_matchingAlgo, "algo",
                              "matching algorithm to use (STANDARD, KNN_MATCH"
                              ", RADIUS_MATCH).")),
      d_k(initData(&d_k, 2, "k",
                   ...
                   "matched together")),
      d_maxDistance(
          initData(&d_maxDistance, .0f, "maxDistance",
                   "maxDistance Threshold for the distance between matched"
                   " descriptors. Distance means here metric distance"
                   "(e.g. Hamming distance), not the distance between "
                   "coordinates (which is measured in Pixels)!")),
...
{
  if (m_dataTracker.hasChanged(d_matcherType)) matcherTypeChanged();
  ...
  if (!m_matches.empty() && !m_matches[0].empty())
  {
    for (std::vector<cv::DMatch>& matchVec : m_matches)
    {
      ...
      for (cv::DMatch& match : matchVec) vec->back().push_back(cvDMatch(match));
    }
  }
  d_matches.endEdit();
...
void DescriptorMatcher::applyFilter(const cv::Mat& in, cv::Mat& out, bool)
{
       ...
       d_trainDescriptors.getValue().type() == 0))
  {
    msg_error("DescriptorMatcher::update")
        << "Cannot match binary descriptors with these settings. either"
           "use BRUTEFORCE matcher or FLANN with LSHIndexParams";
    return;
  }
